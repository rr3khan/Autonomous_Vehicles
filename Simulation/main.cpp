...
int main() {
  ...
  // Create enums for lanes

  enum lane_line { left, center, right};
    ...
    if (length && length > 2 && data[0] == '4' && data[1] == '2') {
      ...
      if (s != "") {
        ...
        if (event == "telemetry") {
          ...
          vector<double> lane_loss = {0,0,0}; // variable to hold the cost of moving into a lane

          // penalize switching lanes
          for (int index = 0; index < lane_loss.size(); index++){
            if (index != lane){
              lane_loss[index] += 1;
            }
          }
          ...
          for (int i = 0; i < sensor_fusion.size(); i++) {
            ...
            if (d < (2+4*lane+2) && d > (2+4*lane-2)){
              ...
              if (check_car_s > car_s && (check_car_s - car_s) < 30){
                //ref_vel = 29.5;
                too_close = true;
                // car is too close to front car hence penallize state
                lane_loss[lane] += 10;
              } 
            }
          }

          // Penalize moving across multiple lanes
          ...
          // check if cars block the path in different lanes
          // check lane on the right
          int on_right = lane + 1;
          for (int i = 0; i < sensor_fusion.size(); i++) {
            // Check if the car is in the same lane as the ego vehicle
            float d = sensor_fusion[i][6];
            if (d < (2+4*on_right+2) && d > (2+4*on_right-2)){
              double vx = sensor_fusion[i][3];
              double vy = sensor_fusion[i][4];
              double check_speed = sqrt(vx*vx + vy*vy);
              double check_car_s = sensor_fusion[i][5];
              ...
              if (check_car_s > car_s && (check_car_s - car_s) < 50){
                //ref_vel = 29.5;
                // lane is blocked hence heavily penalize
                lane_loss[on_right] += 200;
              } 
            }
          }

          // check lane on the left
          int on_left = lane - 1;
          for (int i = 0; i < sensor_fusion.size(); i++) {
            // Check if the car is in the same lane as the ego vehicle
            float d = sensor_fusion[i][6];
            if (d < (2+4*on_left+2) && d > (2+4*on_left-2)){
              double vx = sensor_fusion[i][3];
              double vy = sensor_fusion[i][4];
              double check_speed = sqrt(vx*vx + vy*vy);
              double check_car_s = sensor_fusion[i][5];
          ...
          // set lane based on minimizing costs
          double min_loss = *std::min_element(lane_loss.begin(), lane_loss.end());
          int lane_change = std::distance(lane_loss.begin(), std::find(lane_loss.begin(), lane_loss.end(), min_loss));
          lane = lane_change;

          // Print debug statements
          std::cout << "Left: " << lane_loss[0] << " Middle: " << lane_loss[1] << " Right: " << lane_loss[2] << "\n";
          std::cout << "Lane Change:" << lane_change << std::endl << "\n";
